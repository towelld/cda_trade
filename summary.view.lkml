view: summary {
  ...
  measure: sum_complete_red {
    type: sum
    sql: case when cast(${complete} as decimal)/cast(${total} as decimal)<${data_tolerances.rag_complete_lower} then cast(${complete} as decimal)/cast(${total} as decimal) else 0 end;;
    drill_fields: [details*]
  }
